...
/**
 * @description Indicates the trends of percentage over time to track progress and displayed on home screen
 * @param {Object} homeContext Home context containing context variables for home screen
 * @returns {JSX.Element}
 */
const PercentageChart = ({homeContext}) => {
    ...
    const compassData = useMemo(()=>compass.yearly_compasses ? compassHelper.constructCompassChartData(compass) : [], []);
    ...
    return (
        <Dialog
            isVisible = {chartIndex === chartModes.PERCENTAGE_MODE}
            onBackdropPress={()=>setChartIndex(chartModes.IDLE_MODE)}
            overlayStyle = {dialogStyling}
        >
            <View className=''>
                <Text className='text-slate-200 text-xl mb-2'>Compass Percentage</Text>
                <LineChart
                    data={{
                        datasets: [
                        {
